...
import { log, loggedCall } from './logger.js';
import type { FetchRecipesInput, ManageMealPlanInput, MealType, Cuisine } from './types.js';
...
const server = new McpServer({
  ...
  version: '1.0.0',
  description:
    'Vegetarian protein-rich weekly meal planner with gym/run schedule awareness. ' +
...
server.tool(
  fetchRecipesToolDef.name,
  fetchRecipesToolDef.description,
  {
  ...
  async (input) => {
    ...
    try {
      const result = await fetchRecipes(input as FetchRecipesInput);
      const output = {
...
server.tool(
  manageMealPlanToolDef.name,
  manageMealPlanToolDef.description,
  {
  ...
  async (input) => {
    ...
    const start = Date.now();
    try {
      const result = await manageMealPlan(input as ManageMealPlanInput);
      const summary = result.data ? summarizePlan(result.data as any) : undefined;
    ...
    try {
      ...
      return {
        content: [
          ...
          { type: 'resource', resource: { uri: 'ui://meal-planner/dashboard', mimeType: 'text/html;profile=mcp-app', text: html } } as any,
        ],
      };
    } catch (err: any) {
...
function summarizePlan(data: any): any {
  if (!data?.days) return data;
  const days = Object.entries(data.days).map(([name, day]: [string, any]) => ({
    day: name,
