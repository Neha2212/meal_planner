import { RECIPE_BANK, getRecipes } from '../data/recipeBank.js';
import type { Recipe, FetchRecipesInput, Cuisine, MealType } from '../types.js';

const SPOONACULAR_BASE = 'https://api.spoonacular.com/recipes';

// Map Spoonacular response → internal Recipe format
function mapSpoonacularRecipe(data: any, mealType: MealType): Recipe {
  const nutrition = data.nutrition?.nutrients || [];
  const get = (name: string) =>
    Math.round(nutrition.find((n: any) => n.name === name)?.amount || 0);

  return {
    id: `spoon-${data.id}`,
    name: data.title,
    cuisine: (data.cuisines?.[0]?.toLowerCase() as Cuisine) || 'indian',
    mealType: [mealType],
    ...
    spiceLevel: 'medium',
    ingredients: (data.extendedIngredients || []).map((i: any) => i.name),
...
async function fetchFromSpoonacular(
  ...
  return (json.results || []).map((r: any) =>
    mapSpoonacularRecipe(r, input.mealType)
  );
}

// ─── Main export ──────────────────────────────────────────────────────────

export async function fetchRecipes(input: FetchRecipesInput): Promise<{
  recipes: Recipe[];
  source: 'api' | 'local';
  message: string;
}> {
  ...
  const localRecipes = getRecipes(
    input.mealType,
    input.cuisine,
    input.preferHighProtein ? ['high-protein'] : undefined
  );
  ...
  return {
    recipes: results,
    source: 'local',
    message: `Loaded ${results.length} curated vegetarian recipes from local bank.${
      !apiKey ? ' (Set SPOONACULAR_API_KEY in .env to use live API.)' : ''
    }`,
  };
}
...
export const fetchRecipesToolDef = {
  name: 'fetch_recipes',
  description:
    'Fetches vegetarian protein-rich recipes for a given meal type and cuisine. ' +
