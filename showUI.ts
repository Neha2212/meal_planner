import type { MealPlan, DayPlan, Recipe } from '../types.js';

// ─── Server-side render helpers ───────────────────────────────────────────

function workoutBadge(type: string): { icon: string; label: string; color: string } {
...
function cuisineFlag(cuisine: string): string {
  const map: Record<string, string> = { indian: '🇮🇳', chinese: '🇨🇳', italian: '🇮🇹' };
  return map[cuisine] ?? '🍽️';
}

function spiceDots(level: string): string {
  const filled = level === 'hot' ? 3 : level === 'medium' ? 2 : 1;
  return Array.from({ length: 3 }, (_, i) =>
    `<span class="spice-dot ${i < filled ? 'filled' : ''}">🌶</span>`
  ).join('');
}
...
function mealCardHtml(meal: Recipe, dayName: string, mealType: string): string {
...
function dayCardHtml(plan: MealPlan, dayName: string): string {
  const day: DayPlan = plan.days[dayName];
  if (!day) return '';
...
export function generateDashboardHTML(plan: MealPlan, recipeBank: Recipe[] = []): string {
  ...
  const cuisineCount: Record<string, number> = { indian: 0, chinese: 0, italian: 0 };
  allDays.forEach(d =>
    [d.breakfast, d.lunch, d.dinner].forEach(m => { cuisineCount[m.cuisine] = (cuisineCount[m.cuisine] || 0) + 1; })
  );
...
<script>
  ...
  // ── Plan tab refresh (no full page reload) ───────────────────────────────
  async function refreshPlanTab() {
    try {
      const res = await fetch('/api/plan');
      if (!res.ok) return;
      const newPlan = await res.json();
      Object.assign(planData, newPlan);
      const planList = document.querySelector('.plan-list');
      if (!planList) return;
      const DAYS_ORDER = ['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'];
      planList.innerHTML = DAYS_ORDER.map(d => buildDayCardHtml(newPlan, d)).join('');
    } catch (err) {
      console.error('refreshPlanTab failed:', err);
    }
  }

  function buildDayCardHtml(plan, dayName) {
    const d = plan.days[dayName];
    if (!d) return '';
    const badges  = { gym: { icon:'🏋️', label:'Gym Day', color:'#f97316' }, running: { icon:'🏃', label:'Run Day', color:'#3b82f6' }, rest: { icon:'😴', label:'Rest Day', color:'#6b7280' } };
    const badge   = badges[d.workoutType] || badges.rest;
    const pctCal  = Math.min(100, Math.round(d.totalCalories / d.calorieTarget * 100));
    const pctPro  = Math.min(100, Math.round(d.totalProtein  / d.proteinTarget * 100));
    const mealCol = (meal, type, label, cls) => \`
      <div class="meal-col">
        <div class="meal-type-label \${cls}">\${label}</div>
        \${buildMealCardHtml(meal, dayName, type)}
      </div>\`;
    return \`
    <div class="day-card">
      <div class="day-header">
        ...
        <div class="workout-badge" style="background:\${badge.color}18;color:\${badge.color};border-color:\${badge.color}35">\${badge.icon} \${badge.label}</div>
        <div class="day-macros">
          <div class="macro-bar-row"><span>Calories</span><div class="bar"><div class="fill cal-fill" style="width:\${pctCal}%"></div></div><span class="macro-val">\${d.totalCalories} / \${d.calorieTarget}</span></div>
          <div class="macro-bar-row"><span>Protein</span><div class="bar"><div class="fill pro-fill" style="width:\${pctPro}%"></div></div><span class="macro-val">\${d.totalProtein}g / \${d.proteinTarget}g</span></div>
        </div>
      </div>
      <div class="meals-row">
        \${mealCol(d.breakfast, 'breakfast', '🌅 Breakfast', 'breakfast-label')}
        \${mealCol(d.lunch,     'lunch',     '☀️ Lunch',    'lunch-label')}
        \${mealCol(d.dinner,    'dinner',    '🌙 Dinner',   'dinner-label')}
      </div>
    </div>\`;
  }
  ...
  async function sendChat() {
    ...
    try {
      ...
      if (data.error) {
        ...
        if (data.planUpdated) {
          appendChat('assistant', '✅ Plan updated! Refreshing meal plan tab…');
...
export const showUIToolDef = {
  ...
  description:
    'Generates and returns a rich interactive HTML dashboard of the current weekly meal plan. ' +
