...
import { RECIPE_BANK, getAlternatives } from '../data/recipeBank.js';
...
function selectRecipe(mealType, workoutType, usedIds, cuisinePreference) {
    ...
    const cuisinePool = cuisinePreference
        ? [cuisinePreference]
...
function buildDayPlan(dayName, workoutType, usedIds) {
...
// ─── CRUD operations ──────────────────────────────────────────────────────
export async function manageMealPlan(input) {
    ...
    switch (operation) {
        ...
        case 'create': {
            ...
            for (const dayName of DAYS) {
                const workoutType = profile.workoutSchedule[dayName];
        ...
        case 'update': {
            ...
            const plan = readPlan();
            if (!plan)
                return { success: false, message: 'No plan to update. Run create first.' };
            ...
            if (!dayPlan)
                return { success: false, message: `Day "${day}" not in plan.` };
        ...
        case 'delete': {
            const plan = readPlan();
            if (!plan)
                return { success: false, message: 'No plan to delete.' };
            if (!day) {
                // Delete entire plan
            ...
            const dayPlan = plan.days[day];
            const current = dayPlan[mealType];
            const alternatives = getAlternatives(current, 1);
            if (alternatives.length > 0) {
                dayPlan[mealType] = alternatives[0];
                writePlan(plan);
                return {
                    success: true,
...
export const manageMealPlanToolDef = {
    ...
    description: 'CRUD + shuffle operations on the local meal_plan.json file. ' +
...
//# sourceMappingURL=mealPlanCRUD.js.map